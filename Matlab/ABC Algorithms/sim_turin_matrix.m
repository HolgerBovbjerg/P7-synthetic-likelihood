...
function [P_y, t] = sim_turin_matrix(N, Bw, Ns, T, G0, lambda, sigma_N)
    ...
    %% OUTPUTS:
    % P_y       = Power delay spectrum value: Vector with dimension t(m)* N
    %             the timestamp vector t is used as entry of t(m). 
    % t         = Timestamp vector  
    ...
    % realization. 
    for n = 1:N
        lmax = 5000; % poissrnd(tmax*lambda);   % Number of multipath components, created from the Poisson distribution.
